...
class GeoDB:
    def __init__(self, mode: str, conn, path: str | None = None):
        ...
        self.spatial_loaded = True     # set False when DuckDB extensions cannot load
    ...
    def create_rag_table(self):
        """Standalone rag_chunks DDL — needs NO spatial extension (duckdb core
        supports FLOAT[384]); used when booting in degraded no-spatial mode."""
        if self.mode == "duckdb":
            self.x("""CREATE TABLE IF NOT EXISTS rag_chunks (
                id INTEGER PRIMARY KEY, document_id VARCHAR, title VARCHAR,
                chunk_text VARCHAR, source VARCHAR, tags VARCHAR,
    ...
    def info(self) -> dict:
        ...
        return {"mode": self.mode, "path": self.path,
                "extensions": {"spatial": self.spatial_loaded, "vss": self.vss_loaded},
...
def _ensure_extension(conn, name: str) -> bool:
    """Load a duckdb extension with a 3-tier fallback so a blocked
    extensions.duckdb.org (restricted networks / corporate firewalls / some
    Windows ISPs) never breaks the app:

      1. LOAD           — already installed/cached on this machine
      2. vendored copy  — repo/third_party/duckdb-extensions/<ver>/<platform>/
                            (copied into the user's extension dir, then LOAD)
      3. INSTALL + LOAD — network download (last resort)
    """
    try:
    ...
    try:
        ...
        ver = _dk.__version__
        plat = conn.execute("PRAGMA platform").fetchone()[0]
        vend = _REPO_ROOT / "third_party" / "duckdb-extensions" / ver / plat / f"{name}.duckdb_extension"
        if vend.exists():
            usr = Path.home() / ".duckdb" / "extensions" / ver / plat
            usr.mkdir(parents=True, exist_ok=True)
            import shutil as _sh
            if not (usr / vend.name).exists():
                _sh.copy2(vend, usr / vend.name)
            conn.execute(f"LOAD {name}")
            print(f"✔ duckdb {name} — loaded from vendored repo copy ({plat})")
            return True
    except Exception as exc:
        print(f"vendored duckdb {name} failed ({str(exc)[:120]})")
    try:
        conn.execute(f"INSTALL {name}")
        conn.execute(f"LOAD {name}")
        print(f"✔ duckdb {name} — downloaded & installed from extension repo")
        return True
    except Exception as exc:
        print(f"duckdb extension '{name}' unavailable ({str(exc)[:140]})")
        return False
...
def _open_duckdb(path: Path) -> GeoDB:
    ...
    try:
        ...
        if "lock" in str(exc).lower():
            ...
            raise
    spatial_ok = _ensure_extension(conn, "spatial")
    vss_ok = _ensure_extension(conn, "vss")
    if vss_ok:
        try:
            ...
            pass
    if not spatial_ok:
        print("duckdb WITHOUT 'spatial' — app stays up in in-memory feature mode")
    db = GeoDB("duckdb", conn, path=str(path))
    db.spatial_loaded = spatial_ok
    db.vss_loaded = vss_ok
    return db
